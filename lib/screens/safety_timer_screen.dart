...
class _SafetyTimerTabState extends State<SafetyTimerTab> {
  ...
  final contacts = widget.contacts.map((c) => c.phone).toList();
  ...
  try {
    final response = await http.post(
      ...
      body: jsonEncode({
        "user_id": "bharat",
        "duration": _totalSelected,
        "contacts": contacts,
      }),
    );

    print("START API: ${response.statusCode}");
  } catch (e) {
    print("Start API error: $e");
  }
  ...
  try {
    await http.post(
      Uri.parse("$baseUrl/timer/cancel"),
      headers: {"Content-Type": "application/json"},
      body: jsonEncode({
        "user_id": "bharat",
      }),
